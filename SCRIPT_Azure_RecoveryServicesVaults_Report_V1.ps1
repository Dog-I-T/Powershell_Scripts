...
$ReportFile1 = "$($Company)_Azure_StorageAccount_Report_$(get-date -format dd_MM_yyyy).csv"
$ReportFile2 = "$($Company)_AzureVM_BackupEnabled_Report_$(get-date -format dd_MM_yyyy).csv"
$ReportFile3 = "$($Company)_AzureVM_BackupStatus_Report_$(get-date -format dd_MM_yyyy).csv"
...
function GetAzureVMBackupEnabled {
    ...
    ForEach ($sub in $Subscriptions) {
        ...
        $VMs = Get-AZVM -Status | Where-Object {$_.PowerState -notlike "*deallocated*"} | Select-Object Name,PowerState,ResourceGroupName,location

        ForEach ($vm in $VMs) {

            $BackupStatus = Get-AzRecoveryServicesBackupStatus -Name $vm.Name -ResourceGroupName $vm.ResourceGroupName -Type "AzureVM"
                    
            If ($BackupStatus.VaultId) {
                $RSV = $Backupstatus.VaultId.Split('/')[-1]
            }

            ElseIf ($BackupStatus.BackedUp -eq $false) {
                $RSV = "NA"
            }
